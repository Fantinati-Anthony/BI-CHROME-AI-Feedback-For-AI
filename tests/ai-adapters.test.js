...
describe('shared/ai-adapters registries', () => {
  it('exposes LOCAL_ACTIONS, AI_TARGETS and ALL_BUTTONS', () => {
    expect(Array.isArray(window.MyFb.LOCAL_ACTIONS)).toBe(true);
    expect(Array.isArray(window.MyFb.AI_TARGETS)).toBe(true);
    expect(Array.isArray(window.MyFb.ALL_BUTTONS)).toBe(true);
  });

  it('ALL_BUTTONS = LOCAL_ACTIONS + AI_TARGETS in that order', () => {
    const all = window.MyFb.ALL_BUTTONS;
    const expected = window.MyFb.LOCAL_ACTIONS.concat(window.MyFb.AI_TARGETS);
  ...
  it('every entry has key/slug/label/i18nKey/exportFn/defaultVisible', () => {
    window.MyFb.ALL_BUTTONS.forEach((def) => {
  ...
  it('local actions are visible by default; AI targets are not', () => {
    window.MyFb.LOCAL_ACTIONS.forEach((d) => expect(d.defaultVisible).toBe(true));
    window.MyFb.AI_TARGETS.forEach((d)   => expect(d.defaultVisible).toBe(false));
  });

  it('keys are unique across the registry', () => {
    const keys = window.MyFb.ALL_BUTTONS.map((d) => d.key);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it('AI_ADAPTERS preserves selectors per host', () => {
    const claude = window.MyFb.AI_ADAPTERS.find((a) => a.host === 'claude.ai');
